...
use backend\modules\merchants\models\Merchant;
...
class User extends WbpActiveRecord implements IdentityInterface
{
    ...
    public static function checkAccess($key)
    {
        ...
        return self::find()->where(['key' => $key, 'status' => self::STATUS_ACTIVE])->one();
    }
    ...
    {
        if ($this->parent == 0) {
            return true; //array_keys(Stores::getList('id','title','id'));
        }
        ...
        $tmpArray = [];
        foreach ($permissionArray as $store_id => $array) {
