...
class Adverts extends WbpActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['title', 'status', 'price','price_dollar','price_euro', ], 'required', 'message' => 'Это обязательное поле', 'on' => [WbpActiveRecord::ADMIN_ADD_SCENARIO, WbpActiveRecord::ADMIN_EDIT_SCENARIO]],
...
$arrayPar = [];
    ...
    public function getRoom(){
        $param_value ='';
        $param = AdvertsParametrs::find()->where(['advert_id'=>$this->id, 'parametr_id' => 2])->asArray()->one()['value'];
       if($param){
           $param_value = \backend\modules\parametrs\models\ParametrsValue::findOne($param)->value;
